...
/** The snapshot, as the resolver wants it. Rows are [region, side, |x|, y]. */
const rows = fixture.patches as [string, number, number, number][];
const resolved: ResolvedPatch<number>[] = resolvePatches(
  rows.map(([region, side, absX, y], i) => ({
    ref: i,
    name: region,
...
describe('the spelling three.js actually gives us', () => {
  ...
  it('resolves the runtime spelling to the same roots as the GLB spelling', () => {
    ...
    const runtime = resolvePatches([
      { ref: 'heel', name: 'Heel_region_2', x: 0.087, y: 0.036 },
      { ref: 'medmall', name: 'Medial_malleolusl', x: 0.045, y: 0.073 },
      { ref: 'foveola', name: 'Radial_foveolal', x: 0.299, y: 0.845 },
    ]);
...
describe('window semantics', () => {
  ...
  it('places a single-patch region at the middle, so an unwindowed rule takes it', () => {
    const [only] = resolvePatches([{ ref: 0, name: 'Dorsum of hand', x: 0.27, y: 0.8 }]);
