...
import { resolvePatches, patchesForRoot } from '../../src/lib/neuroSkin';
...
const patches = resolvePatches(
  skin.map((mesh) => {
    ...
    mesh.localToWorld(c);
    return { ref: mesh, name: mesh.name, x: c.x, y: c.y };
  }),
);

const mine = patchesForRoot(patches, figure, root);
const pigment = new THREE.Color(pigmentFor(figure, root));
for (const p of mine) {
  p.ref.material = new THREE.MeshStandardMaterial({
    color: pigment.clone().lerp(new THREE.Color(0xd7a88f), 0.25),
...
for (const p of mine) box.expandByObject(p.ref);
...
document.getElementById('tag')!.textContent =
  `${figure} · ${root} · ${view} · ${mine.length}/${skin.length} parches`;
